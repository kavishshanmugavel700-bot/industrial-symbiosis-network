  ...
  function renderMatches(matches) {
    ...
    matches.forEach(match => {
      ...
      if (match.status === 'pending') {
        ...
        `;
      } else if (match.status === 'confirmed' && match.confirmed_at) {
        actionButtons = `
          <div style="font-size: 0.8rem; color: var(--clr-text-muted); font-weight: 500; text-align: right;">
            Confirmed on: ${new Date(match.confirmed_at).toLocaleDateString()}
          </div>
        `;
      }
