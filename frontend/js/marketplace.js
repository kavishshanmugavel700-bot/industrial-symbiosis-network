...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderListings(listings) {
    ...
    listings.forEach(listing => {
      ...
      if (isMyOwnListing) {
        ...
        actionHtml = `
          <button class="btn btn-primary request-match-btn" data-id="${listing.id}" data-material="${listing.material_type || ''}" style="width: 100%;">
            Request Match
          </button>
        `;
      }
  ...
  async function handleRequestMatch(listingId, button) {
    ...
    // Fetch AI-powered compatibility score from the Flask AI service.
    // Falls back to a safe default (0.90) if the service is unreachable.
    let compatibilityScore = 0.90;
    try {
      const sellerMaterial = button.getAttribute('data-material') || '';
      const buyerFactory   = auth.getFactory();
      const buyerNeeds     = buyerFactory?.production_schedule?.needs_material_type || sellerMaterial;
      if (sellerMaterial) {
        const aiRes = await fetch('http://localhost:5000/compatibility/score', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ materialType: sellerMaterial, msdsText: buyerNeeds })
        });
        if (aiRes.ok) {
          const aiData = await aiRes.json();
          if (typeof aiData.compatibilityScore === 'number') {
            // AI service returns 0-100; backend expects 0-1
            compatibilityScore = Number((aiData.compatibilityScore / 100).toFixed(2));
          }
        }
      }
    } catch (_) {
      // AI service unreachable — proceed with default score
    }
